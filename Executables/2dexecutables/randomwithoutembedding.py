# Creates completely random Graph without an embedding. Pretty worthless currently, as it doesn't really have an outer hull to work with

import numpy as np
import sys
sys.path.insert(0, 'C:/Users/ich/Desktop/Uni/Alternating-Edge-Sum')

from main import main
from util.TwoDGraph import TwoDGraph
from scipy.spatial import Delaunay

attempts = 1
stepsize = 2000

def creategraph(vertexnumber):
    #Step one: create random number of vertices on the inside, done via dirichlet distribution (Idk either) and barycentric coordinates of the hull

    newvertices = np.zeros((vertexnumber, 2))
    for i in range(vertexnumber):
        x = np.random.rand()
        y = np.random.rand()
        newvertices[i,0] = x
        newvertices[i,1] = y

    #Step two: create the graph based of these vertices with Delaunay
    tri = Delaunay(newvertices)

    #Step three: Hand in the faces as they are, but shuffle the order of the vertices to ruin the embedding
    shuffleseed = int(200*np.random.rand()) # So we can reuse graphs with different shuffling
    np.random.seed(shuffleseed)
    neworder = np.arange(vertexnumber)
    np.random.shuffle(neworder) # new order of the vertices
    return TwoDGraph(vertices=newvertices[neworder,:].T, faces=tri.simplices), shuffleseed


if __name__ == '__main__':
    #we first randomize the state. This is ironically so we can revisit graphs later on by reusing the seed because the get_state function is giving me an aneurysm
    seed = int(200 * np.random.rand())
    np.random.seed(seed)

    vertexnumber = int(3 + 10*np.random.rand())
    print(vertexnumber)

    randomized_graph, shuffleseed = creategraph(vertexnumber)

    #for naming convention, I will use the state of the random numpy generator that generates our graph
    main(randomized_graph, "output/2dfolder/randomizedwithoutembedding/Graph_of_seed_" + str(seed) + "_" + str(shuffleseed))
